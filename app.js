...
const REMOTE_PRODUCT_URLS = [
  "https://myhmmt.github.io/El-terminali-mobil/Bilgi.txt", // 1: GitHub Pages
  "https://raw.githubusercontent.com/myhmmt/El-terminali-mobil/main/Bilgi.txt", // 2: raw fallback
  "https://cdn.jsdelivr.net/gh/myhmmt/El-terminali-mobil@main/Bilgi.txt"       // 3: jsDelivr fallback
];
...
async function fetchRemoteProducts(){
  try{
    ...
    if(txt.trim().startsWith('{')){
      ...
      for(const [k,v] of Object.entries(obj)){ if(typeof v==='string') newMap[k]={name:v,price:''}; else newMap[k]={name:v.name||'',price:v.price||''}; }
    ...
    const ts=Date.now(); localStorage.setItem('lastSyncTs', String(ts)); setLastSync(ts);
...
btnOnce.onclick = async ()=>{ if(state.scanning){ stop(); btnOnce.textContent='👉 Tek Okut'; state.singleShot=false; return; } state.singleShot=true; btnOnce.textContent='Okutuluyor...'; if(!state.scanning) await start(); statusEl.textContent='Tek seferlik okuma aktif'; };
$('#btnAdd').onclick  = ()=>{
...
(function boot(){
  ...
  const ts = Number(localStorage.getItem('lastSyncTs')||0); setLastSync(ts || null); setRemoteStatus('—');
  if(!productMap || Object.keys(productMap).length===0){ fetchRemoteProducts(); }
  load();
})();
