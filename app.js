/* ====== Sesler ====== */
const Sounds = {
  beep: new Audio('beep.ogg'),          // TANINDIĞINDA (listeye eklendiğinde)
  accepted: new Audio('accepted.ogg'),  // adet düzenleme vb.
  error: new Audio('error.ogg')         // tanımsız / hata
};
...
let cameraStream = null, singleShot = false, mode='manual';

/* ====== Veri ====== */
let PRODUCTS = JSON.parse(localStorage.getItem('GG_PRODUCTS')||'[]');
let CART     = JSON.parse(localStorage.getItem('GG_CART')||'[]');

/* ====== Init: rozet metnini kalıcı yap ====== */
(function initBadge(){
  if (PRODUCTS.length>0 && datasetBadge){
    datasetBadge.textContent = `Önceki veri • ${PRODUCTS.length} ürün yüklü`;
  }
})();

/* ====== Yardımcılar ====== */
const onlyDigits = s => /^[0-9]+$/.test(s);
function showAlert(m){ alertsEl.textContent = m||''; }
...
/* ====== Kamera (sabit yükseklik CSS ile; burada sadece başlat/durdur) ====== */
async function startCamera(){
  prepareAudioOnce(); mode='camera';
  try{
    ...
    await videoEl.play();
    barcodeInput.blur(); qtyInput.blur(); // klavye açılmasın
    clearAlert();
  }catch(e){ showAlert('Kamera açılamadı: '+e.message); }
}
function stopCamera(){ mode='manual'; if (cameraStream){ cameraStream.getTracks().forEach(t=>t.stop()); cameraStream=null; }}
...
fileInput?.addEventListener('change', async (e)=>{
  ...
  for (const ln of lines){
    ...
    bc = (bc||'').replace(/\D/g,''); if (!bc) continue;
    data.push({ barcode: bc, name, price });
  }
  PRODUCTS = data; localStorage.setItem('GG_PRODUCTS', JSON.stringify(PRODUCTS));
...
/* ====== Okutulan liste (küçük font + Sil sola hizalı) ====== */
const cartList = document.getElementById('cartList');
function saveCart(){ localStorage.setItem('GG_CART', JSON.stringify(CART)); }
function renderCart(){
  cartList.innerHTML=''; const frag = document.createDocumentFragment();
  for (const it of CART){
    ...
    const cqty = document.createElement('input'); cqty.type='text'; cqty.value=String(it.qty); cqty.className='in numeric row-qty'; cqty.style.width='66px';
    cqty.addEventListener('change',()=>{ const v=cqty.value.trim(); if(!onlyDigits(v)||v==='0'){ cqty.value=String(it.qty); try{Sounds.error.currentTime=0;Sounds.error.play();}catch{}; return;} it.qty=Number(v); saveCart();});
    cqty.addEventListener('keydown',(ev)=>{ if(ev.key!=='Enter')return; ev.preventDefault(); const v=cqty.value.trim(); if(!onlyDigits(v)||v==='0'){ try{Sounds.error.currentTime=0;Sounds.error.play();}catch{}; return;} it.qty=Number(v); saveCart(); try{Sounds.accepted.currentTime=0;Sounds.accepted.play();}catch{} barcodeInput.focus(); barcodeInput.select?.(); });

    const actions = document.createElement('div'); actions.className='c-actions';
    const del = document.createElement('button'); del.className='btn ghost'; del.style.textAlign='left'; del.textContent='Sil';
    del.addEventListener('click',()=>{ const i=CART.findIndex(x=>x.barcode===it.barcode); if(i>-1){ CART.splice(i,1); saveCart(); renderCart(); }});
    actions.appendChild(del);

    row.appendChild(cbar); row.appendChild(cname); row.appendChild(cqty); row.appendChild(actions);
...
function addToList({ barcode, qty }){
  ...
  if (ex) ex.qty += qty; else CART.push({ barcode, name, price, qty });
  saveCart(); renderCart();
  updateLastInfo({ name, price });
}

/* ====== Tanımsız Onay (error.ogg) ====== */
...
/* ====== Barkod alanı: canlı önizleme (adet onayını beklemeden) ====== */
barcodeInput.addEventListener('input', ()=>{
  const v = barcodeInput.value.trim();
  if (!onlyDigits(v) || v.length < 8){ updateLastInfo({ name:'(henüz ürün yok)', price:'' }); return; }
  const p = findProduct(v);
  if (p) updateLastInfo({ name:p.name, price:p.price });
  else   updateLastInfo({ name:'(TANIMSIZ ÜRÜN)', price:'' });
});
...
qtyInput.addEventListener('keydown',(ev)=>{
  ...
  const barcode = barcodeInput.value.trim(); let qty=(qtyInput.value||'1').trim();
  if(!onlyDigits(barcode)){ try{Sounds.error.currentTime=0;Sounds.error.play();}catch{}; showAlert('Geçersiz barkod.'); barcodeInput.focus(); barcodeInput.select?.(); return; }
  if(!onlyDigits(qty) || qty==='0'){ qty='1'; }
  const prod = findProduct(barcode);
  if (prod){
    try{ Sounds.beep.currentTime=0; Sounds.beep.play(); }catch{}   // TANINDI (listeye ekleniyor)
...
async function onDecode(barcode){
  ...
  // kamera modunda inputa focus verme
}

/* ====== Arama: 50 → +50 ====== */
let searchResults = [];
function renderSearchChunk(start, count=50){
  const end = Math.min(start+count, searchResults.length); const frag=document.createDocumentFragment();
  ...
  searchList.appendChild(frag); searchList.dataset.page=String((+searchList.dataset.page)+1);
}
function doSearch(q){
  const t=(q||'').toLocaleLowerCase('tr-TR').trim(); searchList.innerHTML=''; searchList.dataset.page='0';
  ...
  searchCount.textContent=`${searchResults.length} sonuç`; renderSearchChunk(0,50);
}
...
searchList.addEventListener('scroll', ()=>{
  ...
  if(!atBottom) return; const page=+searchList.dataset.page||0; const nextStart=page*50;
