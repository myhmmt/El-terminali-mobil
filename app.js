...
function exportPDF(){
  ...
  const rows = codes.map(code=>{
    ...
    const pstr  = productMap[code]?.price||'0,00';
    const price = parseMoney(pstr);
...
function buildSearchIndex(){
  searchArr = Object.entries(productMap).map(([code,obj])=>({code,name:obj.name,price:obj.price,fold:trFold(obj.name||'')}));
}
$('#searchName').addEventListener('input', ()=>{
  const q = trFold(($('#searchName').value||'').trim());
  const list = $('#searchList'); list.innerHTML = ''; if(!q) return;
...
async function start(){
  ...
  try{
    ...
    // 720p@60, arka kamera
    try{
      stream = await tryGet({ video:{ facingMode:{exact:'environment'}, width:{ideal:1280}, height:{ideal:720}, frameRate:{ideal:60,max:90} }, audio:false });
    }catch(_){
      // 720p@60, esnek
      try{
        stream = await tryGet({ video:{ facingMode:{ideal:'environment'}, width:{ideal:1280}, height:{ideal:720}, frameRate:{ideal:60,max:90} }, audio:false });
      }catch(__){
        // 1080p@30
        try{
          stream = await tryGet({ video:{ facingMode:{ideal:'environment'}, width:{ideal:1920}, height:{ideal:1080}, frameRate:{ideal:30,max:60} }, audio:false });
        }catch(___){
          // generic
    ...
    if(e?.name==='NotAllowedError') msg='Kamera izni verilmedi.';
...
function normPriceStr(p){
  ...
  let intPart, frac='';
  if(di>=0){ intPart=only.slice(0,di); frac=only.slice(di+1); }
  else intPart=only;
...
/* =======================
   UZAKTAN VERİ (Bilgi.txt)
   ======================= */
function setRemoteStatus(okOrTxt, txt){
  if(typeof okOrTxt==='string'){ remoteStat.textContent='Uzaktan veri: '+okOrTxt; return; }
  remoteStat.textContent = okOrTxt
    ? 'Uzaktan veri: ✓ ' + (txt||'alındı')
    : 'Uzaktan veri: ✗ ' + (txt||'alınamadı');
}
function setLastSync(ts){
  lastSyncLbl.textContent = ts ? new Date(ts).toLocaleString('tr-TR') : '—';
}
async function fetchWithFallback(url){
  const full = url + (url.includes('?')?'&':'?') + 'v=' + Date.now();
  const res = await fetch(full, { cache:'no-store', mode:'cors', headers:{'cache-control':'no-cache'} });
  if(!res.ok) throw new Error('HTTP ' + res.status);
  return res.text();
}
async function fetchRemoteProducts(){
  try{
    setRemoteStatus('yükleniyor…');
    let txt=null, lastErr=null, used=null;

    for(const u of REMOTE_PRODUCT_URLS){
      try{ used=u; txt=await fetchWithFallback(u); break; }
      catch(e){ lastErr=e; }
    }
    ...
    const ts = Date.now();
    localStorage.setItem('lastSyncTs', String(ts));
    setLastSync(ts);

    setRemoteStatus(true, (used||'').replace(location.origin,''));
    return true;
  }catch(err){
    console.error(err);
    setRemoteStatus(false, err.message||'hata');
    return false;
  }
}
...
btnOnce.onclick = async ()=>{
  ...
  state.singleShot=true; btnOnce.textContent='Okutuluyor...';
...
btnRemote.onclick           = ()=> fetchRemoteProducts();
...
(function boot(){
  ...
  const ts = Number(localStorage.getItem('lastSyncTs')||0);
  setLastSync(ts || null);
  setRemoteStatus('—');

  // yerelde ürün yoksa otomatik uzaktan çek
  if(!productMap || Object.keys(productMap).length===0){
    fetchRemoteProducts();
  }

  load(); // liste/adetleri yükle
})();
